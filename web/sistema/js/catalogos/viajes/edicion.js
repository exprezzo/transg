var Edicionviajes = function(){
	...
	this.guardar=function(){
		...
		var depositos=$(tabId+' .grid_depositos').wijgrid('data');
		...
		$.ajax({
			...
			if ( resp.success == true	){
				...
				var depositos=resp.datos.depositos;
				var gridDep=$(me.tabId+" .grid_depositos");
				var dataDep=gridDep.wijgrid('data');
				dataDep.length=0;
				for(var i=0; i<depositos.length; i++){
					dataDep.push(depositos[i]);
				}
				gridDep.wijgrid('ensureControl', true);
	...
	this.configurarToolbar=function(tabId){					
			...
			$(this.tabId + ' .toolbarEdicion .btnDepositos').click( function(){
				$( me.tabId + ' .frmEdicion.frmGeneral').hide();
				$( me.tabId + ' .consumo').hide();
				$( me.tabId + ' .gastos').hide();
				$( me.tabId + ' .depositos').fadeIn();				
				$(me.tabId+' .grid_depositos').wijgrid('ensureControl',true);				
			});			
	};	
}

...
class conceptoModelo extends Modelo{
	...
	function buscar($params){
		...
		if ($paginar){
			$limit=$params['limit'];
			$start=$params['start'];		
			$sql = 'SELECT *,id as value, DATE_FORMAT(now(), "%d/%m/%Y" ) as fecha FROM '.$this->tabla.$filtros.' ORDER BY nombre limit :start,:limit ';
		}else{			
			$sql = 'SELECT *,id as value, DATE_FORMAT(now(), "%d/%m/%Y" ) as fecha FROM '.$this->tabla.$filtros.' ORDER BY nombre';
		}
		// echo $sql;
		$sth = $con->prepare($sql);
		if ($paginar){
